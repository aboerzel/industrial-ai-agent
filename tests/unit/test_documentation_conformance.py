...
def test_fastapi_boundary_documented_routes_exist_in_api_source() -> None:
    api_source = (PROJECT_ROOT / "src/industrial_ai_agent/infrastructure/api/app.py").read_text(
        encoding="utf-8"
    )
    documentation = (
...
def test_documented_manual_e2e_commands_are_declared_by_frontend_package() -> None:
    ...
    documentation = (
        PROJECT_ROOT / "docs/learning/browser-e2e-journeys.md"
    ).read_text(encoding="utf-8")
...
def test_current_fastapi_adr_marks_legacy_router_as_historical() -> None:
    ...
    assert "The historical `TaskRequirements -> DeterministicModelRouter` path is not the active" in adr
