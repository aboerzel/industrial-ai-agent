...
- **Quality and reliability engineering:** The agent is tested through complete browser journeys, not just individual prompts. Twelve real user journeys with 46 interactions cover multi-step conversations, clicks on stations, products, faults, and documents, German and English, and different access levels. The demanding Real-Model suite is deliberately manual and uses Local Qwen only.
- **End-to-end security and data classification:** Every piece of information keeps its protection level across the system, from the request through tools and the model to the stored investigation and PDF report. Follow-up questions and clicks cannot bypass those boundaries, and higher-protected data does not become visible simply because a conversation continues.
- **Controlled model selection and execution:** Model choice is transparent and predictable. A central catalog and persistent assignments select models that fit the task, the data, and the required capabilities; security checks always take precedence. The system never switches silently to another provider.
- **Authenticated MCP capabilities:** The agent does not receive unrestricted backend access. It works through authenticated MCP services with clearly bounded operations for Factory, Knowledge, and Hardware, while Runtime, Observability, and RCA support diagnostics and development. A service is only considered ready when MCP and its required tools actually work, not merely because a port is open.
- **Hybrid deterministic and AI processing:** The LLM handles interpretation and flexible decisions where they add value. Security, authorization, validation, policy, and state transitions stay deterministic in code, so the AI is used where judgment helps but not where guarantees are required.
- **Human-in-the-loop before protected actions:** The agent cannot execute protected actions on its own. These steps require explicit approval, and the caller must be authorized before that approval can be used; an unauthorized request cannot trigger or consume it.
- **PhysicalDevicePort and closed-loop recovery:** The S04 recovery demonstrator connects the agent to an abstract physical-device layer. Recovery is not just sending a command: the system observes the state, performs the action, and checks whether the equipment actually returned to the desired state. Additional adapters, such as a future `MHSDeviceAdapter`, can be added below this interface; full MHS compliance is not claimed.
- **Durable runtime and configuration integrity:** Investigations, checkpoints, and model configuration survive restarts. Deliberate settings remain in place across upgrades, unintended changes are detected, and services are used only after their actual functionality has been checked.
- **Privacy-conscious observability:** Traces, logs, metrics, and dashboards show which model, tool, or MCP service participated in a run and where a failure occurred. OpenTelemetry, Tempo, Loki, Prometheus, Grafana, and Langfuse provide this visibility without putting prompts, answers, documents, tool payloads, or secrets into diagnostics.
- **Evidence-based root cause analysis:** A root-cause analysis must rest on verifiable information, not merely sound plausible. The system separates observed facts, derived findings, and hypotheses, and prevents the model from filling in missing evidence on its own.

**Trust is not delegated to the LLM.** It comes from verifiable data, clear security boundaries, persistent state, automated regression tests, and observable execution.
