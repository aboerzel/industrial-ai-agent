...
- **Quality and reliability engineering:** The agent is tested through complete user journeys, not only isolated prompts. Deterministic regression tests sit alongside a manually triggered, resumable and fingerprinted Real-Model E2E suite covering 12 browser journeys, 46 user-visible interactions, German and English, four clearance levels, multi-turn conversations, and real DOM/reference clicks. That manual suite uses Local Qwen only and has no external provider dependency.
- **End-to-end security and data classification:** Access rules follow information from the user and API through tools, models, persistence, and PDF export. Authoritative `DataClassification` can move upward with trusted context but never silently down; visibility, protected knowledge access, model Egress, and approval before protected actions remain policy-controlled.
- **Controlled model selection and execution:** A central Model Catalog and persistent assignments make model choice predictable across restarts. `MANUAL` and `AUTO` selection are constrained by the task, data classification, and required capabilities; capability and security/Egress guards remain authoritative, with no silent provider fallback.
- **Authenticated MCP capabilities:** Industrial capabilities are exposed through authenticated MCP services with bounded operations, keeping integrations modular and limiting what the agent can call. Factory, Knowledge, and Hardware serve the main agent path; Runtime, Observability, and RCA support diagnostics and development. Readiness checks verify the MCP protocol and required tools, not merely an open port.
- **Hybrid deterministic and AI processing:** Deterministic code handles security, authorization, validation, policy, and state guarantees. The LLM provides bounded semantic judgment, such as choosing among already allowed tools or explaining an outcome.
- **Human-in-the-loop before protected actions:** Protected side effects never execute automatically; they require explicit approval. The caller is authorized before the one-time resume is claimed, and an unauthorized caller cannot consume that approval.
- **PhysicalDevicePort and closed-loop recovery:** The physical integration is separated behind a `PhysicalDevicePort`, so recovery follows an observe -> act -> verify lifecycle rather than treating a successful command as a successful recovery. Hardware MCP exposes the bounded capability, while a future `MHSDeviceAdapter` can be placed below the port; full MHS compliance and a production MHS implementation are not claimed.
- **Durable runtime and configuration integrity:** Investigations and LangGraph checkpoints persist, while model assignments survive restarts and upgrades without being silently replaced. Configuration drift is detected, and service readiness checks confirm actual protocol availability before use.
- **Privacy-conscious observability:** OpenTelemetry, Tempo, Loki, Prometheus, Grafana, and Langfuse provide traces, metrics, logs, dashboards, and model-oriented visibility. Diagnostics retain safe operational metadata while excluding sensitive prompts, document contents, tool payloads, and secrets.
- **Evidence-based root cause analysis:** RCA is grounded in trusted evidence rather than plausible prose. Evidence Requirements and Evidence Source capabilities control what counts as complete, while `OBSERVED`, `DERIVED`, and `HYPOTHESIS` findings stay separate and the model cannot invent missing evidence.

Trust is not delegated to the LLM. It comes from deterministic security boundaries, structured contracts, trusted evidence, persistent state, automated regression tests, and observable execution.
