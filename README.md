...
- **End-to-end security and data classification:** Access rules follow information from the user and API through tools, models, persistence, and PDF export. A follow-up or export cannot make higher-classified information visible by accident: trusted context may only raise the protection level, never lower it. Knowledge access, model Egress, and approval before protected actions remain policy-controlled.
...
- **Authenticated MCP capabilities:** Industrial capabilities are exposed through authenticated MCP services with bounded operations, keeping integrations modular and limiting what the agent can call. Factory, Knowledge, and Hardware serve the main agent path; Runtime, Observability, and RCA support diagnostics and development. A service is considered ready only when MCP and its required tools actually work, not merely when its port is open.
...
- **Human-in-the-loop before protected actions:** Protected side effects never execute automatically; they require explicit approval. The caller must be authorized before the approval can be used, and an unauthorized caller cannot consume it.
...
- **Durable runtime and configuration integrity:** Investigations and LangGraph checkpoints persist, while model assignments survive restarts and upgrades without being silently replaced. Unintended configuration changes are detected, and service readiness checks confirm actual protocol availability before use.
...
- **Evidence-based root cause analysis:** RCA is grounded in trusted facts rather than plausible prose. The investigation can only be completed when the required evidence has been observed; `OBSERVED`, `DERIVED`, and `HYPOTHESIS` findings stay separate, and the model cannot invent what is missing.
