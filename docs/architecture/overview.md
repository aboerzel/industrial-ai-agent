...
Requirements are proportional to trusted investigation type: station discovery requires none, station status requires `CURRENT_MACHINE_STATE`, and station troubleshooting requires `CURRENT_MACHINE_STATE`, `ACTIVE_FAULT`, and `RELEVANT_FAULT_DOCUMENTATION`. An active fault must come from a machine-state observation; fault documentation must carry an explicit trusted relevant-fault reference. Model prose, user identifiers, query text, and arbitrary retrieved content cannot mark a requirement complete. The checkpointable LangGraph state persists requirement IDs, safe source correlations, satisfied/missing IDs, and effective classification, but never raw tool payloads. A normal final draft is accepted only when the reconstructed ledger is complete. Otherwise the graph returns a compact, response-language-aware missing-evidence runtime instruction with understandable observation descriptions and canonical IDs to the existing model-decision loop. Tool and repeated-finalization bounds produce `EVIDENCE_REQUIREMENTS_UNSATISFIED` with orchestration failure attribution when evidence remains incomplete. Physical recovery continues to use its separate ADR-018 lifecycle invariants.

During evidence acquisition, the application resolves declarative source capabilities rather than routing to concrete tools. `CURRENT_MACHINE_STATE -> machine_state_observation -> get_machine_status` is the current registration chain; a future admitted tool may provide the same source. For each incomplete ledger, prerequisite-ready sources capable of missing requirements are intersected with authorized/admitted tools before the model decision. Thus unrelated authorized tools are absent during acquisition, while the model chooses among all eligible implementations. `fault_documentation_retrieval` requires `ACTIVE_FAULT`; unrelated documentation does not satisfy the ledger. No eligible admitted source produces the distinct orchestration outcome `EVIDENCE_SOURCE_UNAVAILABLE`; the existing budget outcome remains `EVIDENCE_REQUIREMENTS_UNSATISFIED`.
...
Compose readiness for every deployed MCP service (Factory, Knowledge, Hardware, Runtime,
Observability, and RCA) is protocol-aware: it requires a Streamable HTTP connection,
authenticated session, MCP initialization, `tools/list`, and discovery of the service's
required bounded capabilities, rather than only an open TCP port. `agent-api` waits at
startup only for its applicable Factory, Knowledge, and Hardware dependencies. Its MCP
provider also makes bounded discovery retries for a
